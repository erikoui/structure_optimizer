...
const ConcreteMember = (props) => {
    ...
    // Cross section
    const shp = useMemo(() => {
        const membShape = new THREE.Shape().moveTo(props.points[0][0], props.points[0][1]);
        for (let i = 1; i < props.points.length; i++) {
            membShape.lineTo(props.points[i][0], props.points[i][1]);
        }
        return membShape;
    }, [props.points]);
    ...
    var extrudeSettings = {
        bevelEnabled: false,
        steps: 1,
        depth: props.end[2]-props.start[2]
    };
...
const makeConcretesArray = (modelDb) => {
    ...
    if (modelDb.cols) {
        for (let i = 0; i < modelDb.cols.length; i++) {
            ...
            concretes.push(<ConcreteMember start={col.start} end={col.end} points={col.points} key={keyCounter++} />)
        }
    }else{
...
const makeFEArray = (modelDb) => {
    ...
    for (let i = 0; i < modelDb.FEnodes.length; i++) {
        ...
        mesh.push(<FElabel text={"N" + modelDb.FEnodes[i].name} position={[modelDb.FEnodes[i].coords[0] + offset, modelDb.FEnodes[i].coords[1] + offset, modelDb.FEnodes[i].coords[2] + offset]} />);
    }
    // Draw members
...
const makeSolutionArray = (modelDb) => {
    ...
    // Draw nodes
    for (let i = 0; i < modelDb.FEnodes.length; i++) {
        mesh.push(<FENode c={[modelDb.node_results[i].x+modelDb.node_results[i].dx*d_factor, modelDb.node_results[i].y+modelDb.node_results[i].dy*d_factor, modelDb.node_results[i].z+modelDb.node_results[i].dz*d_factor]} size={nodesize} key={keyCounter++} color="green" />)
    }
    // Draw members
    for (let i = 0; i < modelDb.FEmembers.length; i++) {
        // find start,end
        ...
        for (let j = 0; j < modelDb.FEnodes.length; j++) {
            if (modelDb.FEnodes[j].name === modelDb.FEmembers[i].from) {
                fromCoords = [modelDb.node_results[j].x+modelDb.node_results[j].dx*d_factor, modelDb.node_results[j].y+modelDb.node_results[j].dy*d_factor, modelDb.node_results[j].z+modelDb.node_results[j].dz*d_factor];
                found1 = true;
            }
            if (modelDb.FEnodes[j].name === modelDb.FEmembers[i].to) {
                toCoords = [modelDb.node_results[j].x+modelDb.node_results[j].dx*d_factor, modelDb.node_results[j].y+modelDb.node_results[j].dy*d_factor, modelDb.node_results[j].z+modelDb.node_results[j].dz*d_factor];
                found2 = true;
            }
            if (found1 && found2) {
                console.log("found start and fin")
